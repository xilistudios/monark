/**
 * Site copy. One entry per locale; every page is rendered from these strings.
 * `prefix` drives the sub-path the locale is published under ("" for the default).
 */

export const REPO = "https://github.com/xilistudios/monark";
export const RELEASES = `${REPO}/releases/latest`;
export const ISSUES = `${REPO}/issues`;
export const LICENSE = `${REPO}/blob/main/LICENSE`;

export const LOCALES = [
	{ code: "en", label: "English", native: "EN", prefix: "" },
	{ code: "es", label: "Español", native: "ES", prefix: "es" },
	{ code: "pt", label: "Português", native: "PT", prefix: "pt" },
	{ code: "fr", label: "Français", native: "FR", prefix: "fr" },
	{ code: "de", label: "Deutsch", native: "DE", prefix: "de" },
	{ code: "zh", label: "中文", native: "ZH", prefix: "zh" },
	{ code: "ja", label: "日本語", native: "JA", prefix: "ja" },
];

/** Language names written in the locale itself, for the "other languages" list. */
...
export const locales = {
	en: {
		...
		data: {
			...
			title: "Why Monark asks for Google Drive, and what it can see.",
			body: "Synchronising to the cloud is optional and off by default. If you turn it on and choose Google Drive, Monark requests exactly one restricted scope — the smallest one that makes file-level backup possible.",
			scopeLabel: "Requested scope",
			bullets: [
				["Access is strictly limited", "The scope lets Monark create, open and update only the backup files it created itself. Nothing else in your Drive is reachable."],
				["No access to other files", "Monark cannot read, list, modify or delete any other document, photo or folder in your Google Drive."],
				["Encrypted before upload", "The backup is encrypted on your device with your master password first. Neither Google nor the developers hold the key."],
				["Never sold or shared", "Xilistudios does not collect, sell, transfer or use Google API data for advertising, profiling or training AI models."],
			],
			footPrefix: "Full detail, including the Google Limited Use commitments, is in our",
		...
		privacy: {
			...
			sections: [
				...
				},
				{
					...
					html: `<p>Before any backup file is transmitted to your Google Drive, Monark applies state-of-the-art cryptographic standards implemented in the secure Rust core:</p>
					<ul>
