...
mod tests {
    ...
    fn test_crypto_error_variants() {
        ...
        assert_eq!(
            format!("{}", crypto_err),
            "Argon2 hashing failed: invalid password"
        );
        ...
        assert_eq!(
            format!("{}", crypto_err),
            "Argon2 parameter error: output is too short"
        );
        ...
        assert_eq!(
            format!("{}", crypto_err),
            "Argon2 hashing error: output is too short"
        );
        ...
        assert_eq!(
            format!("{}", crypto_err),
            "ChaCha20Poly1305 encryption/decryption failed"
        );
        ...
        assert_eq!(
            format!("{}", crypto_err),
            format!("HMAC calculation failed: {}", hmac_err_msg)
        );
        ...
        assert_eq!(
            format!("{}", crypto_err),
            "Base64 encoding/decoding failed: Invalid input length: 0"
        );
        ...
        let crypto_err = CryptoError::InvalidNonceLength {
            expected: 12,
            actual: 8,
        };
        assert_eq!(
        ...
        let crypto_err = CryptoError::InvalidSaltLength {
            expected: 16,
            actual: 8,
        };
        assert_eq!(
            format!("{}", crypto_err),
            "Invalid salt length: expected 16, got 8"
        );
        ...
        assert_eq!(
            format!("{}", crypto_err),
            format!("Random number generation failed: {}", rng_err_msg)
        );
    }
}
