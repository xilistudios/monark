...
impl LocalStorageProvider {
    ...
    fn file_to_storage_file(&self, path: &Path) -> StorageResult<StorageFile> {
        ...
        #[cfg(unix)]
        storage_metadata.insert("permissions".to_string(),
        ...
        Ok(StorageFile {
            ...
            name,
            path: path_str,
            size: if is_folder { None } else { Some(metadata.len()) },
            created_at: metadata.created().ok().map(|dt| dt.into()),
            modified_at: metadata.modified().ok().map(|dt| dt.into()),
            is_folder,
            mime_type: if is_folder { Some("application/x-directory".to_string()) } else { None },
            parent_id: path.parent().and_then(|p| {
                p.strip_prefix(&self.base_path).ok().map(|rp| format!("/{}", rp.display()))
            }),
...
impl StorageProvider for LocalStorageProvider {
    ...
    async fn create_file(&self, request: CreateFileRequest) -> StorageResult<StorageFile> {
        ...
        fs::write(&file_path, &request.content).await
            .map_err(StorageError::Io)?;
...
impl LocalStorageProvider {
    async fn search_files_recursive(
        ...
        while let Some(entry) = read_dir.next_entry().await
            ...
            if let Ok(storage_file) = self.file_to_storage_file(&path) {
                if storage_file.name.to_lowercase().contains(&query.to_lowercase()) {
