	...
	const handleAuthenticate = async (provider: StorageProvider) => {
		...
		try {
			...
			if (provider.provider_type === StorageProviderType.GOOGLE_DRIVE) {
				console.log(
					"[CloudStorageSettings] Getting OAuth URL for Google Drive provider",
				);
				...
				console.log(
					"[CloudStorageSettings] OAuth URL received, storing in Redux",
				);
				console.log("[CloudStorageSettings]   - Provider:", provider.name);
				console.log(
					"[CloudStorageSettings]   - State:",
				...
				console.log(
					"[CloudStorageSettings] OAuth state stored in Redux successfully",
				);
			} else {
					...
					<Modal isOpen={oauthState.isOpen} onClose={handleOAuthCancel}>
						<OAuthFlow
							...
							onCredentialsImported={async (code, state) => {
								try {
									console.log(
										"[CloudStorageSettings] Processing imported credentials",
									);
									if (!oauthState.providerName) {
									...
									console.log(
										"[CloudStorageSettings] Imported credentials processed successfully",
									);
								} catch (error) {
									console.error(
										"[CloudStorageSettings] Failed to process imported credentials:",
										error,
									);
									throw error;
								}
