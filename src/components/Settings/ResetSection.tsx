	...
	return (
		<section className='flex items-center gap-4 mt-8'>
			...
			<Modal isOpen={modalOpen} onClose={() => setModalOpen(false)}>
				<div role='alertdialog' aria-modal='true' aria-labelledby='clearDataConfirmTitle'>
					<h2 id='clearDataConfirmTitle' className='font-bold text-lg mb-2'>
						{t('clearDataConfirmTitle', 'Clear All App Data?')}
					</h2>
					<p className='mb-4'>{t('clearDataConfirmDesc', 'This will erase all app data. This action cannot be undone.')}</p>
					{clearError && <div className='alert alert-error mb-2'>{clearError}</div>}
					{clearSuccess && <div className='alert alert-success mb-2'>{t('clearDataSuccess', 'App data cleared successfully.')}</div>}
					<div className='flex gap-2'>
						<button
							...
							autoFocus
						>
							{clearLoading ? t('clearDataLoading', 'Clearing...') : t('clearDataConfirmYes', 'Yes, clear')}
						</button>
						<button
