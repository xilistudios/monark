...
export default function ThemeSwitcher() {
  ...
  useEffect(() => {
    ...
    const storedExpiration = localStorage.getItem(`${THEME_STORAGE_KEY}-expiry`);
    
    const mediaQuery = window.matchMedia('(prefers-color-scheme: dark)');
    setIsSystemPreferredDark(mediaQuery.matches);

    if (storedTheme && storedExpiration && new Date() < new Date(storedExpiration)) {
  ...
  return (
    <div className="dropdown dropdown-end" role="navigation" aria-label="Theme Selection">
      <div 
        tabIndex={0} 
        role="button" 
      ...
      >
        <svg className="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 21a4 4 0 01-4-4V5a2 2 0 012-2h4a2 2 0 012 2v12a4 4 0 01-4 4zm0 0h12a2 2 0 002-2m-2 2a2 2 0 01-2-2m2-2a2 2 0 00-2-2h-2m-4-6h4m-8 6V7m0 0H5m4 0h4" />
        </svg>
        <span className="hidden md:inline">Theme</span>
        <svg className="ml-1 h-3 w-3" viewBox="0 0 24 24" fill="none" stroke="currentColor">
