			...
			<div className="px-1">
				<div className="flex flex-wrap gap-2 mb-3">
					{tags.map((tag) => (
						<span
							key={tag}
							className={`badge badge-sm font-semibold py-2.5 px-3 rounded-lg border ${
								editMode
									? "badge-primary"
									: "bg-base-200/60 border-base-300/60 text-base-content/85"
							}`}
						>
							{editMode && (
								<svg
					...
					{!editMode && tags.length === 0 && (
						<p className="text-base-content/40 italic text-xs">
							{t("vault.tags.noTags")}
						</p>
					)}
				</div>
