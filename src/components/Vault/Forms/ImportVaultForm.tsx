...
export const ImportVaultForm = ({
	...
	const [selectedCloudVault, setSelectedCloudVault] = useState<Vault | null>(null);
	...
	const authenticatedProviders = providers.filter(provider =>
	...
	const loadCloudVaults = async () => {
		...
		try {
			const vaults = await VaultManager.getInstance().listCloudVaults(providerId);
	...
	const handleImportVault = async () => {
		...
		try {
			...
			if (importSource === 'cloud') {
				...
				try {
					const vaultInstance = VaultManager.getInstance().getInstance(selectedCloudVault.id);
					if (!vaultInstance) {
					...
					setError(t("importVault.errors.invalidPassword") || 'Invalid password or failed to access cloud vault');
					return;
				}
			} else {
				// Import local vault
			...
			const newVault: Vault = {
				...
				cloudMetadata: importSource === 'cloud' ? selectedCloudVault.cloudMetadata : undefined,
				volatile: {
					credential: password,
					entries: [],
					navigationPath: '/',
	...
	return (
		<div className="space-y-4">
			...
			{importSource === 'cloud' && (
				<div className="form-control">
					<label className="label">
						<span className="label-text">{t("vaultSelector.selectProvider")}</span>
					</label>
					{authenticatedProviders.length > 0 ? (
						...
						<div className="alert alert-warning">
							...
							<div>
								...
								<p className="text-sm opacity-80">{t("vaultSelector.goToSettings")}</p>
							</div>
						</div>
					)}
				</div>
			...
			{importSource === 'cloud' && providerId && (
				<div className="form-control">
					<label className="label">
						<span className="label-text">{t("vaultSelector.selectCloudVault")}</span>
					</label>
					{loadingCloudVaults ? (
