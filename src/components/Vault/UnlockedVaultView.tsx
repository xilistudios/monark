...
export interface UnlockedVaultViewProps {
	...
	t: (key: string) => string;
	onEditEntry?: (entry: DataEntry) => void;
	onEditGroup?: (entry: GroupEntry) => void;
}
...
function UnlockedVaultView({
	...
	const {
		...
		selectedEntry: contextSelectedEntry,
	} = useContext(VaultModalContext)!;
	useEffect(() => {
		// When the edit entry modal is requested, set the selected entry and switch to edit mode
		if (isEditEntryModalOpen && contextSelectedEntry) {
			setSelectedEntry(contextSelectedEntry);
			setSidebarMode("edit");
			setIsMobileSidebarOpen(true);
		}
	}, [isEditEntryModalOpen, contextSelectedEntry]);
	useEffect(() => {
		setSidebarMode("view");
	...
	const handleEditGroup = (entry: GroupEntry) => {
		setSelectedEntry(entry);
		setSidebarMode("edit");
		// Open sidebar on mobile when editing
		setIsMobileSidebarOpen(true);
	...
	return (
		<div
			data-testid="unlocked-vault-view"
			className={`flex flex-col h-full transition-opacity duration-300 ${isVisible ? "opacity-100" : "opacity-0"}`}
		>
