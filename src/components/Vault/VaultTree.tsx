...
import { Entry, isDataEntry, isGroupEntry } from '../../interfaces/vault.interface';
          ...
          <div key={node.id} className="my-2">
            <div className="flex items-center justify-between bg-base-200 p-2 rounded">
              ...
              <div className="flex gap-1">
                {isGroupEntry(node) && (
                  <>
                    <button className="btn btn-xs btn-primary" onClick={() => onAddEntry(node.id)}>
                      + Entry
                    </button>
                    <button className="btn btn-xs btn-secondary" onClick={() => onAddGroup(node.id)}>
                      + Group
                    </button>
                  </>
                )}
                <button className="btn btn-xs btn-accent" onClick={() => onEdit(node)}>
                  Edit
                </button>
                <button 
                ...
                >
                  {deletingId === node.id ? <span className="loading loading-spinner loading-xs"></span> : 'Delete'}
                </button>
              </div>
            </div>
          </div>
