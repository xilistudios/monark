...
export class CloudStorageCommands {
	...
	static async listProviders(): Promise<StorageProvider[]> {
		try {
			...
			throw this.handleError(error, 'Failed to list providers');
		}
	}
	...
	static async addProvider(request: AddProviderRequest): Promise<void> {
		try {
			...
			throw this.handleError(error, 'Failed to add provider');
		}
	}
	...
	static async removeProvider(name: string): Promise<void> {
		try {
			...
			throw this.handleError(error, 'Failed to remove provider');
		}
	}
	...
	static async setDefaultProvider(name: string): Promise<void> {
		try {
			...
			throw this.handleError(error, 'Failed to set default provider');
		}
	}
	...
	static async authenticateProvider(providerName?: string): Promise<void> {
		try {
			...
			throw this.handleError(error, 'Failed to authenticate provider');
		}
	}
	 ...
	 */
	static async listCloudVaults(providerName?: string): Promise<CloudVaultMetadata[]> {
		try {
			const files = await invoke<StorageFile[]>('list_vaults', { providerName });
			return files.map(file => this.storageFileToCloudVaultMetadata(file, providerName));
		} catch (error) {
			throw this.handleError(error, 'Failed to list cloud vaults');
		}
	}
	...
	static async updateCloudVault(
		...
		try {
			...
			throw this.handleError(error, 'Failed to update cloud vault');
		}
	}
	 ...
	 */
	static async deleteCloudVault(request: DeleteCloudVaultRequest): Promise<void> {
		try {
			...
			throw this.handleError(error, 'Failed to delete cloud vault');
		}
	}
	 ...
	 */
	static async listFiles(request?: ListFilesRequest): Promise<ListFilesResponse> {
		try {
	 ...
	 */
	static async readFile(fileId: string, providerName?: string): Promise<ArrayBuffer> {
		try {
			const content = await invoke<number[]>('read_file', { fileId, providerName });
	 ...
	 */
	static async createFolder(request: CreateFolderRequest, providerName?: string): Promise<StorageFile> {
		try {
			return await invoke<StorageFile>('create_folder', { request, providerName });
		} catch (error) {
	 ...
	 * @param providerName - Optional provider name
	 */
	static async deleteFolder(folderId: string, providerName?: string): Promise<void> {
		try {
			await invoke('delete_folder', { folderId, providerName });
		} catch (error) {
			throw this.handleError(error, 'Failed to delete folder');
		}
	}
	 ...
	 */
	static async getFileInfo(fileId: string, providerName?: string): Promise<StorageFile> {
		try {
			return await invoke<StorageFile>('get_file_info', { fileId, providerName });
		} catch (error) {
	 ...
	 */
	static async searchFiles(params: CloudStorageSearchParams): Promise<CloudStorageSearchResponse> {
		try {
			...
			throw this.handleError(error, 'Failed to search files');
		}
	}
	 ...
	 */
	private static storageFileToCloudVaultMetadata(file: StorageFile, providerName?: string): CloudVaultMetadata {
		return {
			id: file.id,
			name: file.name,
	 ...
	 */
	private static handleError(error: any, defaultMessage: string): CloudStorageError {
		const message = error?.message || String(error);
		
		// Determine error type based on message content
		let errorType: CloudStorageErrorType = CloudStorageErrorType.OPERATION_FAILED;
