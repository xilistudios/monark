...
export default class VaultCommands {
	 ...
	 */
	static async writeCloudVault(request: CreateCloudVaultRequest): Promise<string> {
	...
	static async readVault(
		...
		return this.read(vaultPath, password);
	}
	...
	static async writeVault(
		...
		// Otherwise, treat as local vault
		await this.write(vaultPath, password, vaultContent);
		return vaultPath;
	}
	 ...
	 * @param providerName - Optional provider name for cloud vaults
	 */
	static async deleteVault(vaultPath: string, providerName?: string): Promise<void> {
		// If provider name is provided, assume it's a cloud vault
		if (providerName) {
			await this.deleteCloudVault({
				vaultId: vaultPath,
				providerName
			});
		} else {
