...
describe('ImportVaultForm with Cloud Storage', () => {
  ...
  describe('Cloud Vault Selection', () => {
    ...
    it('should show loading state while fetching cloud vaults', async () => {
      ...
      await waitFor(() => {
        expect(screen.queryByText('Loading cloud vaults...')).not.toBeInTheDocument()
      }, { timeout: 200 })
    })
    ...
    it('should show message when no cloud vaults found', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('No vaults found in cloud storage')).toBeInTheDocument()
      })
    })
    ...
    it('should auto-fill vault name when cloud vault is selected', async () => {
      ...
      await waitFor(() => {
        const nameInput = screen.getByPlaceholderText('Enter vault name (optional)')
  ...
  describe('Error Handling', () => {
    it('should show error when cloud vault listing fails', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/Error: Failed to load vaults/)).toBeInTheDocument()
      })
    })
