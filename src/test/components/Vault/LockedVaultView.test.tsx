...
describe("LockedVaultView", () => {
	...
	it("renders correctly for local vault", () => {
		...
		expect(
			screen.getByPlaceholderText("vault.unlock.passwordPlaceholder"),
		).toBeInTheDocument();
		expect(
			screen.getByRole("button", { name: "vault.unlock.unlockButton" }),
		).toBeInTheDocument();
	});
	...
	it("renders cloud badge for cloud vaults", () => {
		...
		expect(
			screen.getByText(
				"This vault is stored in the cloud. It will be downloaded when unlocked.",
			),
		).toBeInTheDocument();
	});
	...
	it("does not render cloud badge for local vaults", () => {
		...
		expect(
			screen.queryByText("This vault is stored in the cloud"),
	...
	it("does not show cloud unlock message when not provided", () => {
		...
		expect(
			screen.queryByText("Downloading from cloud"),
		).not.toBeInTheDocument();
	});
	...
	it("calls setPassword when password input changes", async () => {
		...
		const passwordInput = screen.getByPlaceholderText(
			"vault.unlock.passwordPlaceholder",
		);
	...
	it("calls handleUnlockVault when unlock button is clicked", async () => {
		...
		const unlockButton = screen.getByRole("button", {
			name: "vault.unlock.unlockButton",
		});
