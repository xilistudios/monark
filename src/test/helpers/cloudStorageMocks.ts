...
export function setupMockProvider(
	...
	mockInvoke.mockImplementation((command: string, args?: any) => {
		...
		if (
			command === "authenticate_provider" &&
			(args?.providerName === providerName || !args?.providerName)
		) {
			return Promise.resolve(undefined);
		}
		...
		if (
			command === "list_vaults" &&
			(!args?.providerName || args?.providerName === providerName)
		) {
			return Promise.resolve(mockStorageFiles);
		}
		...
		return Promise.reject(
			new Error(
				`Provider ${providerName} not configured for command: ${command}`,
			),
		);
	});
}
