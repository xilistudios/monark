...
import { type VaultInstance, VaultManager } from "../../services/vault";
...
describe("Cloud Vault Integration", () => {
	...
	beforeEach(() => {
		...
		vi.mocked(CloudStorageCommands.listProviders).mockImplementation(
			async () => {
				await mockInvoke("list_providers");
				return mockProviders;
			},
		);
		vi.mocked(CloudStorageCommands.addProvider).mockImplementation(
			async (request) => {
				await mockInvoke("add_provider", { request });
				return undefined;
			},
		);
		vi.mocked(CloudStorageCommands.removeProvider).mockImplementation(
			async (name) => {
				await mockInvoke("remove_provider", { name });
				return undefined;
			},
		);
		vi.mocked(CloudStorageCommands.setDefaultProvider).mockImplementation(
			async (name) => {
				await mockInvoke("set_default_provider", { name });
				return undefined;
			},
		);
		vi.mocked(CloudStorageCommands.authenticateProvider).mockImplementation(
			async (providerName) => {
				await mockInvoke("authenticate_provider", { providerName });
				return undefined;
			},
		);
		vi.mocked(CloudStorageCommands.listCloudVaults).mockImplementation(
			async (providerName) => {
		...
		vi.mocked(CloudStorageCommands.writeCloudVault).mockImplementation(
			async (requestOrVaultId: any, vaultContent?: any, providerName?: any) => {
				// Handle both request object and positional parameters
				if (typeof requestOrVaultId === "string") {
					// Positional form: (vaultId, vaultContent, providerName)
	...
	describe("Provider Management Flow", () => {
		...
		it("should set default provider", async () => {
			...
			expect(
				verifyInvokeCall("set_default_provider", {
		...
		it("should remove provider", async () => {
			...
			expect(
				verifyInvokeCall("remove_provider", { name: "google-drive-secondary" }),
			).toBe(true);
		});
		...
		it("should handle provider authentication flow", async () => {
			...
			expect(
				verifyInvokeCall("authenticate_provider", {
			...
			expect(state.providerStatus["google-drive-primary"]).toBe(
				"authenticated",
			);
		});
		...
		it("should handle authentication failure", async () => {
			...
			await expect(
				vaultManager.authenticateProvider("google-drive-primary"),
			).rejects.toThrow();
	...
	describe("Cloud Vault Import Flow", () => {
		it("should import existing cloud vault", async () => {
			...
			const importedVaults = await vaultManager.listCloudVaults(
				"google-drive-primary",
			);

			// Verify cloud vaults were listed
			expect(
				verifyInvokeCall("list_vaults", {
					providerName: "google-drive-primary",
				}),
			).toBe(true);
		...
		it("should list cloud vaults from provider", async () => {
			const vaults = await CloudStorageCommands.listCloudVaults(
				"google-drive-primary",
			);
	...
	describe("Cloud Vault Operations", () => {
		...
		it("should verify changes are synced to cloud", async () => {
			...
			const writeCall = callHistory.find(
				(call) => call.command === "write_cloud_vault",
			);
		...
		it("should handle multiple operations in sequence", async () => {
			...
			const callHistory = getInvokeCallHistory();
			const writeCalls = callHistory.filter(
				(call) => call.command === "write_cloud_vault",
			);
	...
	describe("Vault Sync Operations", () => {
		...
		it("should handle sync for non-cloud vault", async () => {
			...
			await expect(localVaultInstance.syncWithCloud()).rejects.toThrow(
		...
		it("should handle sync for locked vault", async () => {
			...
			await expect(lockedVaultInstance.syncWithCloud()).rejects.toThrow(
				"Vault must be unlocked to sync",
			);
		});
	});
	...
	describe("Performance and Optimization", () => {
		it("should handle large vault content efficiently", async () => {
			...
			vi.mocked(CloudStorageCommands.readCloudVault).mockImplementation(() =>
				Promise.resolve(largeVaultContent),
			);
