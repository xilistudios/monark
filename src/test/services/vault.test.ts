...
import type { CloudVaultMetadata, StorageProvider } from '../../interfaces/cloud-storage.interface'
...
import { CloudStorageErrorType, StorageProviderType, type GoogleDriveConfig } from '../../interfaces/cloud-storage.interface'
...
describe('VaultInstance', () => {
	...
	describe('unlock', () => {
		it('should unlock a local vault successfully', async () => {
			...
			expect(VaultCommands.read).toHaveBeenCalledWith('test-path', 'test-password')
			expect(mockDispatch).toHaveBeenCalledWith(
				vaultActions.setVaultCredential({ vaultId: 'test-vault-id', credential: 'test-password' })
			)
			expect(mockDispatch).toHaveBeenCalledWith(
		...
		it('should unlock a cloud vault successfully', async () => {
			...
			const cloudVaultInstance = new VaultInstance(cloudVault, mockDispatch, mockGetState)

			const mockVaultContent = {
				entries: [{ id: 'entry1', type: 'note', title: 'Test Entry' }],
			}
			mockCloudStorageCommands.readCloudVault.mockResolvedValue(mockVaultContent)
		...
		it('should throw error if unlock fails', async () => {
			vi.mocked(VaultCommands.read).mockRejectedValue(new Error('Invalid password'))

			await expect(vaultInstance.unlock('wrong-password')).rejects.toThrow('Failed to unlock vault: Error: Invalid password')
		})
	})
	...
	describe('lock', () => {
		it('should lock the vault', () => {
			...
			expect(mockDispatch).toHaveBeenCalledWith(vaultActions.lockVault('test-vault-id'))
		})
	})

	describe('addEntry', () => {
		it('should add entry to root level', async () => {
			const newEntry: Entry = { id: 'new-entry', type: 'note', title: 'New Entry' }
			const mockVaultContent = {
			...
			expect(mockDispatch).toHaveBeenCalledWith(
				vaultActions.setVaultEntries({ vaultId: 'test-vault-id', entries: [newEntry] })
			)
		})
		...
		it('should throw error if parent group not found', async () => {
			...
			await expect(vaultInstance.addEntry(['nonexistent-group'], newEntry)).rejects.toThrow(
				'Parent group not found at path: nonexistent-group'
			)
		})
	})
	...
	describe('syncWithCloud', () => {
		it('should sync cloud vault successfully', async () => {
			...
			vi.mocked(CloudStorageCommands.readCloudVault).mockResolvedValue(mockVaultContent)
			...
			expect(mockDispatch).toHaveBeenCalledWith(vaultActions.syncCloudVault('test-vault-id'))
		})

		it('should throw error for non-cloud vault', async () => {
			await expect(vaultInstance.syncWithCloud()).rejects.toThrow('Cannot sync non-cloud vault')
		})
	})
...
describe('VaultManager', () => {
	...
	describe('loadProviders', () => {
		it('should load providers successfully', async () => {
			...
			vi.mocked(CloudStorageCommands.listProviders).mockResolvedValue(mockProviders)

			await vaultManager.loadProviders()

			expect(CloudStorageCommands.listProviders).toHaveBeenCalled()
			expect(mockDispatch).toHaveBeenCalledWith(vaultActions.setStorageProviders(mockProviders))
		})

		it('should throw error if loading fails', async () => {
			vi.mocked(CloudStorageCommands.listProviders).mockRejectedValue(new Error('Network error'))

			await expect(vaultManager.loadProviders()).rejects.toThrow('Failed to load providers: Error: Network error')
		})
	})
	...
	describe('authenticateProvider', () => {
		it('should authenticate provider successfully', async () => {
			vi.mocked(CloudStorageCommands.authenticateProvider).mockResolvedValue(undefined)

			await vaultManager.authenticateProvider('google-drive')

			expect(mockDispatch).toHaveBeenCalledWith(
				vaultActions.setProviderStatus({ providerId: 'google-drive', status: 'authenticating' })
			)
			expect(CloudStorageCommands.authenticateProvider).toHaveBeenCalledWith('google-drive')
			expect(mockDispatch).toHaveBeenCalledWith(
				vaultActions.setProviderStatus({ providerId: 'google-drive', status: 'authenticated' })
			)
		})

		it('should handle authentication failure', async () => {
			vi.mocked(CloudStorageCommands.authenticateProvider).mockRejectedValue(new Error('Auth failed'))

			await expect(vaultManager.authenticateProvider('google-drive')).rejects.toThrow(
				'Failed to authenticate provider: Error: Auth failed'
			)

			expect(mockDispatch).toHaveBeenCalledWith(
				vaultActions.setProviderStatus({ providerId: 'google-drive', status: 'error' })
			)
		})
	})
	...
	describe('listCloudVaults', () => {
		it('should list cloud vaults successfully', async () => {
			...
			vi.mocked(CloudStorageCommands.listCloudVaults).mockResolvedValue(mockCloudVaults)
	...
	describe('createVault', () => {
		it('should create cloud vault successfully', async () => {
			...
			vi.mocked(CloudStorageCommands.writeCloudVault).mockResolvedValue(cloudVaultId)
		...
		it('should create local vault successfully', async () => {
			...
			const vaultId = await vaultManager.createVault('Local Vault', 'password', 'local', undefined, 'local.vault')
		...
		it('should throw error if provider ID missing for cloud vault', async () => {
			await expect(vaultManager.createVault('Test', 'password', 'cloud')).rejects.toThrow(
				'Provider ID is required for cloud vaults'
			)
		})
	})
		...
		describe('Cloud Storage Integration', () => {
          ...
          config: {
            ...
            redirect_uri: 'test',
				...
				await vaultManager.addProvider(config)
				const vaultId = await vaultManager.createVault('Cloud Vault', 'password', 'cloud', 'provider_123')
      ...
      it('should handle cloud vault operations with token expiration', async () => {
        ...
        await expect(vaultInstance!.addEntry([], newEntry)).rejects.toThrow();
			...
			it('should handle cloud vault migration scenarios', async () => {
				...
				vi.mocked(CloudStorageCommands.writeCloudVault).mockResolvedValue('migrated-vault-id')
			...
			it('should handle concurrent cloud vault operations', async () => {
				...
				const vaultInstance = vaultManager.getInstance('concurrent-cloud-vault')

				// Mock successful operations
				vi.mocked(CloudStorageCommands.updateCloudVault).mockResolvedValue(undefined)
