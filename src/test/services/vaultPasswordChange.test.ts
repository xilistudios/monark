...
describe('Vault Password Change', () => {
  ...
  describe('VaultInstance.changePassword', () => {
    ...
    it('should change password for local vault successfully', async () => {
      ...
      const vaultInstance = new VaultInstance(localVault, mockDispatch, mockGetState);
    ...
    it('should throw error if vault is not unlocked', async () => {
      ...
      const vaultInstance = new VaultInstance(lockedVault, mockDispatch, mockGetState);
      
      await expect(vaultInstance.changePassword('new-password')).rejects.toThrow(
    ...
    it('should throw error if cloud vault update fails', async () => {
      ...
      await expect(vaultInstance.changePassword('new-password')).rejects.toThrow(
        'Failed to change vault password: Cloud storage error'
      );
    });
    ...
    it('should throw error if local vault update fails', async () => {
      ...
      await expect(vaultInstance.changePassword('new-password')).rejects.toThrow(
        'Failed to change vault password: File system error'
      );
    });
  });
  ...
  describe('VaultManager.changeVaultPassword', () => {
    it('should change password through VaultManager', async () => {
      const changePasswordSpy = vi.spyOn(VaultInstance.prototype, 'changePassword');
