...
describe("Vault State Persistence Integration", () => {
	let store: ReturnType<
		typeof configureStore<{ vault: ReturnType<typeof vaultReducer> }>
	>;
	...
	describe("Error Handling", () => {
		it("should handle settings store save errors gracefully", async () => {
			...
			expect(consoleSpy).toHaveBeenCalledWith(
				"Failed to save vault state to settings:",
				expect.any(Error),
			);
		...
		it("should handle settings store load errors gracefully", async () => {
			...
			expect(result).toEqual({});
			expect(consoleSpy).toHaveBeenCalledWith(
				"Failed to load vault state from settings:",
				expect.any(Error),
			);
		...
		it("should handle initialization errors gracefully", async () => {
			const consoleSpy = vi
				.spyOn(console, "error")
				.mockImplementation(() => {});
			mockSettingsStore.get.mockRejectedValue(
			...
			expect(consoleSpy).toHaveBeenCalledWith(
				"Failed to initialize vault state:",
				expect.any(Error),
			);
	...
	describe("Data Integrity", () => {
		...
		it("should handle Date serialization correctly", async () => {
			...
			expect(restored.savedVaults![0].lastAccessed).toEqual(
				new Date("2025-01-20T15:30:00.000Z"),
			);
		});
	});
});
